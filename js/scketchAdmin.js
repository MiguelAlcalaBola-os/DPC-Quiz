...
var firebaseConfig = {
  apiKey: "AIzaSyCR4bO6XCrtoJoTG-77PrnUFtF-RF87m-M",
  authDomain: "http://dpc-quiz.42web.io/",
  projectId: "dpc-quizbd",
  storageBucket: "http://dpc-quiz.42web.io/",
  messagingSenderId: "584535024565",
  appId: "1:584535024565:web:87fa0fdf38a5c8408baffc",
  measurementId: "G-8Z6YEGWWVQ",
};
// Initialize Firebase
...
let fond, fond1, fond2, fond3, fond4, fond5;
...
class User {
  ...
  getMindMaps() {
    ...
    mindMapsRef.once("value").then((snapshot) => {
      if (snapshot) {
  ...
  deleteAccount() {
    const deleteConfirmed = confirm(
      "Are you sure you want to delete your account & all of your saved mindmaps?!"
    );
...
class MindMap {
  ...
  save() {
    if (!this.name) {
      // if this is the new mind map without a name:
      const mindMapName = prompt(
        "Ingresa el nombre de tu mapa mental para que sea guardado bajo esos caracteres específicos (procura no olvidar el nombre ni los caracteres de tu mapa mental)"
      );
      if (mindMapName) {
        this.name = mindMapName;
      } else {
        alert(
          "Usted debe asignar un nombre a tu mapa mental para que este pueda ser guardado"
        );
        return;
      }
    }
    // putting the mind map into database:
    const mindMapRef = firebase
      .database()
      .ref(currentUser.uid + "/mindmaps/" + this.name);
    mindMapRef.set({
      name: this.name,
    ...
    alert(
      "You saved " +
        this.name +
        " mind map in your user's database! If you want to open it in the future, press open button and input the name."
    );
  ...
  new() {
    if (currentUser.isLogged) {
      ...
      let needSave = confirm(
        "Do you want to save changes to the current mind map?"
      );
      if (needSave) {
        this.save();
      }
  ...
  open() {
    ...
    if (currentUser.mindmaps.length) {
      ...
      // ask for name:
      let inputedMindMapName = prompt(
        `Input the name of your saved mind map. There are the names of saved mind maps in your storage: ${savedMindMapsNames}.`
      );
      ...
      if (inputedMindMapName) {
        ...
        alert(
          "You need to input some saved mind map name... or nothing will be opened..."
        );
      }
    } else {
      alert(
        "There is no saved mind maps yet... Create a new one, save it and then try to open it ;-)"
      );
    }
  }
  delete() {
    const ok = confirm(
      "Are you sure you want to delete this mind map forever?"
    );
    if (ok) {
      const mindMapRef = firebase
        .database()
  ...
  draw() {
    ...
    fill("yellow");
    rect(
      this.selectedKeyword.x,
      this.selectedKeyword.y,
      this.selectedKeyword.w + 7,
      this.selectedKeyword.h + 7,
      5
    );
    // draw keywords:
    for (let i = this.keywords.length; i > 0; i--) {
      ...
      line(
        keyword.x,
        keyword.y,
        this.keywords[keyword.parentIndex].x,
        this.keywords[keyword.parentIndex].y
      );
...
function preload() {
  fond1 = loadImage("http://127.0.0.1/cuestionario/img/fond1.jpg");
  fond2 = loadImage("http://127.0.0.1/cuestionario/img/fond2.jpg");
  fond3 = loadImage("http://127.0.0.1/cuestionario/img/fond3.jpg");
  fond4 = loadImage("http://127.0.0.1/cuestionario/img/fond4.jpg");
  fond5 = loadImage("http://127.0.0.1/cuestionario/img/fond5.jpg");
}
...
keywordInput.addEventListener("input", (e) => {
  if (settingsChanger.value !== "mindmap") {
    ...
    alert(
      "You can't change selected keyword text in mind map settings mode! If you want to change selected keyword text, change settings for on keyword."
    );
  }
});
...
let selectFondo = document.getElementById("selectfon");
selectFondo.addEventListener("click", () => {
  let selectfon = document.getElementById("select-imagen");
  if (selectfon.value === "fondo 1") {
    fond = fond1;
  } else if (selectfon.value === "fondo 2") {
    fond = fond2;
  } else if (selectfon.value === "fondo 3") {
    fond = fond3;
  } else if (selectfon.value === "fondo 4") {
    fond = fond4;
  } else if (selectfon.value === "fondo 5") {
    fond = fond5;
  }
});
...
deleteKeywordBtn.addEventListener("click", () => {
  if (settingsChanger.value === "keyword") {
    ...
    alert(
      "You can't delete keywords in mind map settings mode! If you want to delete selected keyword, change settings for on keyword."
    );
  }
});
...
newMindMapBtn.addEventListener("click", () => {
  if (currentUser.isLogged) {
    ...
    alert(
      "You need to sign in (or sign up) if you want to initiate a new mind map!"
    );
  }
});
...
openBtn.addEventListener("click", () => {
  if (currentUser.isLogged) {
    ...
    alert(
      "You need to sign in (or sign up) if you want to open one of your saved mind maps!"
    );
  }
});
